...
function getAllBlocks() {
  const manual = Array.isArray(state.blocks) ? state.blocks : [];
  const auto = state.autoHolidaysEnabled && Array.isArray(state.autoHolidayBlocks) ? state.autoHolidayBlocks : [];
  return [...manual, ...auto].sort((a, b) => a.start.localeCompare(b.start));
}

function getBlocksWithinPhasePeriod() {
  if (!state.phaseDays.length || !state.endDate) return getAllBlocks();
  const start = state.phaseDays[0];
  const end = state.endDate;
  return getAllBlocks().filter((block) => block.start <= end && block.end >= start);
}
...
function renderReport(hoursPerDay, totalHours, mode) {
  ...
  `).join('');
  const reportBlocks = getBlocksWithinPhasePeriod();
  const blocksHtml = reportBlocks.length
    ...
    : '<p>Nenhum bloqueio dentro do periodo da fase pratica.</p>';
...
function drawPdfBlocks(pdf, margin, contentWidth, y, pageHeight, senaiLogoDataUrl) {
  ...
  if (!reportBlocks.length) {
    pdf.text('Nenhum bloqueio dentro do periodo da fase pratica.', margin, y);
    return y + 8;
  }
